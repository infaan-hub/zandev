...
function createTextFall(root, canvas, params) {
  ...
  function svgToImage(markup) {
    const blob = new Blob([markup.trim()], { type: "image/svg+xml;charset=utf-8" });
  ...
  function buildTextMask(value, fitW, fitH) {
    ...
    for (const lines of candidates) {
      ...
      if (!best || score > best.score) best = { lines, w: boxW, h: boxH, scale, score };
    }
  ...
  async function ensureMask() {
    ...
    if (trimmed) {
      ...
      maskLines = 1;
      mask = await svgToImage(mode === "mobile" ? LOGO_SVG_MOBILE : LOGO_SVG_DESKTOP);
    }
    return mask;
  }
  ...
  async function sampleTargets() {
    ...
    const ox = params.align === "left" ? fitBox.minX : fitBox.minX + (fitW - dispW) / 2;
  ...
  function computeNearPosition(t) {
    ...
    const maxDist = Math.min(jitter, t.tx - bounds.minX, bounds.maxX - t.tx, t.ty - bounds.minY, bounds.maxY - t.ty);
  ...
  function spawnParticles(targets) {
    return targets.map((t, i) => ({
      ...
      hx: t.tx, hy: t.ty, tx: t.tx, ty: t.ty,
      fx: t.tx, fy: t.ty, sx: t.tx, sy: t.ty,
      delay: (i % 20) / 20, done: false,
    }));
  }

  function maybeScramble(p, ratePerSec = 0.9, dt = 0.016) {
    if (Math.random() < ratePerSec * dt) p.char = randChar();
  }

  function updateIdle(p, dt) { clampParticle(p); maybeScramble(p, 0.9, dt); }
  ...
  function applyMagnetic(p, dt) {
    if (params.strength > 0 && mouse.active) {
      ...
      const radius = (params.reach / 100) * 0.5 * Math.min(canvas.width, canvas.height);
  ...
  function updateFall(p, dt) {
    ...
    p.x += p.vx * dt;
    p.y += p.vy * dt;
    if (p.y >= floorY) { p.y = floorY; p.vy *= -0.25; p.vx *= 0.8; }
    clampParticle(p);
  }
  ...
  function startForm(now) {
    phase = "form";
    formStart = now;
    particles.forEach((p) => { p.fx = p.x; p.fy = p.y; p.done = false; });
  }
  ...
  function draw(now) {
    ...
    for (const p of particles) {
      if (phase === "idle") { updateIdle(p, dt); applyMagnetic(p, dt); }
      else if (phase === "fall") updateFall(p, dt);
      else if (phase === "form") updateForm(p, now, dt);
      else if (phase === "spread") updateSpread(p, now, dt);
      else if (phase === "done") { applyMagnetic(p, dt); maybeScramble(p, 0.9, dt); }
      ctx.fillText(p.char, p.x, p.y);
    }
    if (phase === "form" && particles.every((p) => p.done)) {
      phase = "done";
      particles.forEach((p) => { p.x = p.tx; p.y = p.ty; p.hx = p.tx; p.hy = p.ty; });
    }
  ...
  function ensureLoop() { if (!rafId && visible) rafId = requestAnimationFrame(draw); }
  ...
  function onClick() {
    ...
    if (phase === "done") { startSpread(performance.now()); ensureLoop(); return; }
    triggerFall();
  }
  ...
  function onPointerOut() { mouse.active = false; }
  ...
  async function reset() {
    ...
    if (wasSettled) {
      phase = "done";
      particles.forEach((p) => { p.x = p.tx; p.y = p.ty; p.hx = p.tx; p.hy = p.ty; p.done = true; });
    } else {
      phase = "idle";
    }
    lastTime = 0;
  ...
  function scheduleResize() {
    ...
    resizeTimer = window.setTimeout(() => { void reset(); }, 120);
  }
...
const CURSOR_DEFAULTS = { strength: 150, reach: 30 };

export default function TextFall({
  text = "TEXT FALL",
  color = "#00FFC1",
  glyphFont = { fontFamily: "Menlo", fontWeight: 400 },
  align = "center",
  size = 90,
  glyphSize = 10,
  speed = 50,
  ...
  const glyphFamily = (glyphFont && glyphFont.fontFamily) || "ui-monospace, SFMono-Regular, Menlo, monospace";
  ...
  useLayoutEffect(() => {
    ...
    const engine = createTextFall(root, canvas, {
      text, glyphFamily, color, align, autoFall, glyphSize,
      textSize: size, speed, gravity, scatter,
      strength: cur.strength, reach: cur.reach,
    });
    return () => engine.destroy();
  }, [text, glyphFamily, color, align, autoFall, size, glyphSize, speed, gravity, scatter, cur.strength, cur.reach]);
